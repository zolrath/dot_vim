...
VIMRC_FILE = 'vimrc'
VUNDLE_FILE = 'settings/VundlePackages.vim'
...
namespace :vim do 
  ...
  task :vundle do
    begin
      ...
      puts "Couldn't clone Vundle. Are you connected to the internet?"
    end
  end
  desc "Install Vundle, and symlink dotfiles"
  task :init => ["link", "vundle"] do
    puts "Linked and Vundled! Open up vim and run :BundleInstall to complete installation."
  end
  desc "Install Nailgun server for VimClojure"
  task :clojure do
    begin
      sh %{make -C lib/vimclojure-nailgun-client}
      puts "Built the Nailgun Server for VimClojure. You should use the 
    Lein-Nailgun plugin (https://github.com/ibdknox/lein-nailgun) to start it."
    rescue
      puts "Could not build the Nailgun Server."
    end
  end
end
